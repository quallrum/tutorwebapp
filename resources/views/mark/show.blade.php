	...
	<section class="container-fluid journal">
		<div class="journal__group">
			...
			@can('journal.changeGroup', $group)
				<a class="journal__group-button" href="{{ route('mark.group') }}">Изменить группу</a>
			@endcan
		</div>
		<div class="journal__subject">
			...
			@can('journal.changeSubject', $group)
				<a class="journal__subject-button" href="{{ route('mark.subject', ['group'=>$group->id]) }}">Изменить предмет</a>
			@endcan
		</div>
		...
		@can('mark.edit')
			<div class="journal__buttons">
				...
				<a class="journal__download" href="{{ route('mark.file', ['group' => $group->id, 'subject' => $subject->id]) }}" download="download">скачать</a>
			</div>
			</form>
		@endcan
	</section>

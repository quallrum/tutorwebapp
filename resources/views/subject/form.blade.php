...
@section('content')
	<section class="container-fluid adminEditSubject">
		<form action="{{ $action }}" method="post" name="adminEditSubjectNameType">
			<input type="hidden" name="_token" value="{{ csrf_token() }}"/>
			@method($method)
			<div class="adminEditSubject__heading">
				<input class="adminEditSubject__group" type="text" name="title" value="{{ $subject->title }}" autofocus="autofocus"/>
				<select class="adminEditSubject__type" name="type" id="adminEditSubjectType">
					@foreach ($types as $type)
						@if ($subject->type and $subject->type->id == $type->id)
